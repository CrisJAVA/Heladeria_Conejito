...
public class DataInitializer implements CommandLineRunner {
        ...
        public void run(String... args) {
                ...
                if (nivelFidelizacionRepository.count() > 0) {
                        ...
                        String[][] fakeUsers = {
                                        { "María García", "maria@email.com", "987654321", "Jr. Lima 456, Ica", "Plata",
                                                        "350", "450" },
                                        { "Carlos López", "carlos@email.com", "976543210", "Av. Grau 789, Ica", "Oro",
                                                        "520", "1200" },
                                        { "Ana Martínez", "ana@email.com", "965432109", "Calle Real 321, Ica",
                                                        "Diamante", "850", "2500" },
                                        { "Pedro Sánchez", "pedro@email.com", "954321098", "Av. Los Maestros 111, Ica",
                                                        "Plata", "200", "380" },
                                        { "Lucía Torres", "lucia@email.com", "943210987", "Jr. Ayacucho 222, Ica",
                                                        "Bronce", "45", "80" },
                                        { "Roberto Díaz", "roberto@email.com", "932109876",
                                                        "Av. Municipalidad 333, Ica", "Oro", "600", "1500" },
                        };
                ...
                if (metodoPagoRepository.findByNombreIgnoreCase("Puntos").isEmpty()) {
                        MetodoPago mp = new MetodoPago();
                        mp.setNombre("Puntos");
                        mp.setIcono("stars");
                        metodoPagoRepository.save(mp);
                ...
                if (productoRepository.count() == 0) {
                        ...
                        java.util.function.Function<String, String> img = t -> "https://placehold.co/400x300/ffd9df/a43756?text="
                                        + t;

                        // --- HELADOS (en cono) ---
                        if (helados != null) {
                                guardarProducto(helados, "Chocolate", "Helado suave y cremoso en cono", 7.00, 50, true,
                                                true, img.apply("Helado+Chocolate"));
                                guardarProducto(helados, "Vainilla", "Helado suave y cremoso en cono", 7.00, 50, true,
                                                true, img.apply("Helado+Vainilla"));
                        }

                        // --- RASPADILLAS ---
                        if (raspadillas != null) {
                                guardarProducto(raspadillas, "Raspadilla de Lúcuma", "Sabor tradicional y cremoso",
                                                5.00, 80, true, true, img.apply("Raspadilla+Lucuma"));
                                guardarProducto(raspadillas, "Raspadilla de Fresa",
                                                "Dulce y frutal, ideal para refrescarte", 5.00, 80, true, true,
                                                img.apply("Raspadilla+Fresa"));
                                guardarProducto(raspadillas, "Raspadilla de Coco", "Suave, tropical y delicioso", 5.00,
                                                60, true, false, img.apply("Raspadilla+Coco"));
                                guardarProducto(raspadillas, "Raspadilla de Menta",
                                                "Fresca y aromática, perfecta para el calor", 5.00, 60, true, false,
                                                img.apply("Raspadilla+Menta"));
                                guardarProducto(raspadillas, "Raspadilla de Maracuyá", "Tropical y ligeramente ácida",
                                                5.00, 70, true, true, img.apply("Raspadilla+Maracuya"));
                                guardarProducto(raspadillas, "Raspadilla de Piña",
                                                "Dulce y tropical, llena de frescura", 5.00, 70, true, false,
                                                img.apply("Raspadilla+Pina"));
                                guardarProducto(raspadillas, "Raspadilla de Limón", "Cítrica y refrescante", 5.00, 75,
                                                true, true, img.apply("Raspadilla+Limon"));
                                guardarProducto(raspadillas, "Raspadilla de Chicha",
                                                "Sabor tradicional con un toque único", 5.00, 65, true, true,
                                                img.apply("Raspadilla+Chicha"));
                        }

                        // --- BEBIDAS ---
                        if (bebidas != null) {
                                guardarProducto(bebidas, "Chicha Morada Helada",
                                                "Bebida tradicional peruana a base de maíz morado", 7.00, 100, true,
                                                true, img.apply("Chicha+Morada"));
                        }

                        // --- PIZZAS ---
                        if (pizzas != null) {
                                guardarProducto(pizzas, "Pizza Personal",
                                                "Ideal para una persona. Práctica y deliciosa", 18.00, 30, true, true,
                                                img.apply("Pizza+Personal"));
                                guardarProducto(pizzas, "Pizza Mediana", "Perfecta para compartir entre 3 a 4 personas",
                                                32.00, 25, true, true, img.apply("Pizza+Mediana"));
                                guardarProducto(pizzas, "Pizza Familiar",
                                                "Para compartir en grande. Ideal para 5 a 6 personas", 45.00, 20, true,
                                                true, img.apply("Pizza+Familiar"));
                        }
                }
                ...
                if (promocionRepository.count() == 0) {
                        for (Object[] p : new Object[][] {
                                        { "2x1 en Conos",
                                                        "Todos los lunes y martes, llévate 2 conos del sabor que quieras por el precio de 1.",
                                                        "2x1", "Lun - Mar", "ice-cream", "#ff6b9d" },
                                        { "Combo Familiar",
                                                        "2 pizzas + 4 raspadillas a un precio especial. Ideal para compartir en familia.",
                                                        "S/ 39.90", "Todos los días", "users", "#ffd93d" },
                                        { "Helado de Cumpleaños",
                                                        "Celebra con nosotros y recibe un helado gigante completamente gratis presentando tu DNI.",
                                                        "GRATIS", "En tu cumpleaños", "cake", "#c8b6ff" },
                                        { "Noche de Pizzas",
                                                        "Los jueves por la noche, todas las pizzas tienen 25% de descuento.",
                                                        "-25%", "Jueves 7:00 pm", "percent", "#f97316" },
                        }) {
                ...
                if (seccionLandingRepository.count() == 0) {
                        for (Object[] s : new Object[][] {
                                        { "helados", "Helados Artesanales",
                                                        "Suaves y cremosos en cono, preparados al momento",
                                                        "https://images.unsplash.com/photo-1567206563064-6f60f40a2b57?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080",
                                                        "#ff6b9d", "#ff8fab" },
                                        { "raspadillas", "Raspadillas Refrescantes",
                                                        "8 sabores tradicionales para combatir el calor de Ica",
                                        ...
                                        { "pizzas", "Pizzas Artesanales",
                                                        "Personal, mediana o familiar, horneadas al momento",
                                                        "https://images.unsplash.com/photo-1513104890138-7c749659a591?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080",
                                                        "#a7e4f2", "#c3ecf6" },
                                        { "chicha", "Chicha Morada Helada",
                                                        "La bebida tradicional peruana, siempre fría y deliciosa",
