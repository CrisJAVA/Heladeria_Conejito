...
export default function CartPage() {
  ...
  const [orderResult, setOrderResult] = useState<{ codigo: string; total: number } | null>(null);
  ...
  const handlePlaceOrder = async (numeroOperacion?: string) => {
    ...
    try {
      const res = await crearPedido({
        metodoEntregaId,
        metodoPagoId,
        direccionEntrega: deliveryMethod === "delivery" ? (user?.direccion || "") : undefined,
        numeroOperacion,
      ...
      setOrderResult({ codigo: res.codigoPedido, total: res.total });
      ...
      <div className="min-h-screen bg-[#fffbf7] flex flex-col relative">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <Link to="/menu" className="inline-flex px-8 py-3 bg-gradient-to-r from-[#ff6b9d] to-[#ff8fab] text-white rounded-full font-medium shadow-md hover:shadow-lg transition-all">
              Explorar menú
            </Link>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-[#fffbf7] flex flex-col relative">
        ...
        <main className="flex-1 flex items-center justify-center px-4">
          <div className="w-full max-w-md bg-white rounded-3xl p-8 shadow-lg border border-gray-100 text-center">
            ...
            <div className="bg-[#fffbf7] rounded-2xl p-6 mb-6 space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-[#2d2d2d]">{orderResult.codigo}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Total</span>
                <span className="font-bold text-[#ff6b9d]">S/.{orderResult.total.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Estado</span>
                <span className="px-3 py-1 bg-yellow-100 text-yellow-700 rounded-full text-xs font-medium">PENDIENTE</span>
              </div>
            </div>
            <div className="flex flex-col gap-3">
              <button onClick={() => navigate("/mis-pedidos")} className="w-full py-3 bg-gradient-to-r from-[#ff6b9d] to-[#ff8fab] text-white rounded-xl font-semibold shadow-md hover:shadow-lg transition-all">
                Ver mis pedidos
              </button>
              <button onClick={() => navigate("/menu")} className="w-full py-3 bg-gray-100 text-gray-600 rounded-xl font-medium hover:bg-gray-200 transition-all">
                Seguir comprando
              </button>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-[#fffbf7] flex flex-col relative">
      ...
      <header className="sticky top-0 z-40 bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm">
        <div className="flex items-center justify-between px-4 md:px-8 py-4 max-w-4xl mx-auto w-full">
          <button onClick={() => paymentStep === "select" ? navigate(-1) : setPaymentStep("select")} className="flex items-center gap-2 text-gray-600 hover:text-[#ff6b9d] transition-colors font-medium">
            <ArrowLeft className="w-5 h-5" /> Volver
          </button>
          ...
          <button onClick={clearCart} className="text-sm text-red-500 hover:text-red-600 font-medium">Vaciar</button>
        </div>
      </header>
      ...
      <main className="flex-1 px-4 md:px-8 py-6 max-w-4xl mx-auto w-full">
        {paymentStep === "select" && (
          <>
            ...
            <div className="space-y-4 mb-10">
              {items.map((item) => (
                <div key={item.name} className="bg-white rounded-2xl p-4 flex gap-4 relative group transition-all hover:scale-[1.005] border border-gray-100 shadow-sm hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start">
                      ...
                      <button onClick={() => removeFromCart(item.name)} className="text-gray-400 hover:text-red-500 transition-colors p-1">
                        <Minus className="w-4 h-4" />
                      </button>
                    </div>
                    <div className="flex items-center justify-between mt-4">
                      <div className="flex items-center bg-gray-100 rounded-full px-1 py-1">
                        <button onClick={() => updateQuantity(item.name, -1)} className="w-8 h-8 flex items-center justify-center text-[#ff6b9d] font-bold hover:bg-gray-200 rounded-full transition-colors">
                          <Minus className="w-4 h-4" />
                        </button>
              ...
              <div className="space-y-8">
                <section className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                  ...
                  <div className="space-y-2 border-b border-gray-100 pb-4 mb-4">
                    ...
                    <div className="flex justify-between text-gray-500 text-sm">
                      <span>Delivery</span>
                      <span className="font-semibold text-[#2d2d2d]">{deliveryMethod === "delivery" ? `S/.${SHIPPING_COST.toFixed(2)}` : "S/.0.00"}</span>
                    </div>
                  </div>
                    ...
                    {[
                      ...
                      <label key={m.id} onClick={() => setDeliveryMethod(m.id)} className={`flex items-center gap-4 p-4 border rounded-2xl cursor-pointer transition-all ${deliveryMethod === m.id ? "border-[#ff6b9d] bg-[#ffd9df]" : "border-gray-200 bg-white hover:bg-gray-50"}`}>
                        ...
                        <div className="flex flex-col">
                          <span className="font-semibold text-[#2d2d2d]">{m.label}</span>
            ...
            <div className="text-center">
              <button
                ...
                className={`py-4 px-12 rounded-2xl font-bold text-lg transition-all active:scale-95 shadow-lg w-full md:w-auto ${paymentMethod ? "bg-[#ff7e9d] text-[#761235] hover:shadow-xl cursor-pointer" : "bg-gray-100 text-gray-400 cursor-not-allowed"}`}
              >
        ...
        {paymentStep === "yape" && (
          <div className="max-w-md mx-auto bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
            <div className="text-center mb-6">
              <div className="w-20 h-20 rounded-2xl bg-[#6b3fa0] flex items-center justify-center mx-auto mb-4 shadow-lg">
            ...
            <div className="bg-[#f8f5ff] rounded-2xl p-4 mb-6 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-[#2d2d2d]">Heladería Conejito</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Celular</span>
                <span className="font-semibold text-[#2d2d2d]">999 999 999</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Total a pagar</span>
                <span className="font-bold text-[#ff6b9d]">S/.{total.toFixed(2)}</span>
              </div>
            </div>
            ...
            <button disabled={!yapeOp.trim() || processing} onClick={() => handlePlaceOrder(yapeOp.trim())} className="w-full py-3 bg-gradient-to-r from-[#ff6b9d] to-[#ff8fab] text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {paymentStep === "plin" && (
          <div className="max-w-md mx-auto bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
            <div className="text-center mb-6">
              <div className="w-20 h-20 rounded-2xl bg-[#00b8a9] flex items-center justify-center mx-auto mb-4 shadow-lg">
            ...
            <div className="bg-[#f0fffd] rounded-2xl p-4 mb-6 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-[#2d2d2d]">999 888 777</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Total a pagar</span>
                <span className="font-bold text-[#ff6b9d]">S/.{total.toFixed(2)}</span>
            ...
            <button disabled={!plinOp.trim() || processing} onClick={() => handlePlaceOrder(plinOp.trim())} className="w-full py-3 bg-gradient-to-r from-[#ff6b9d] to-[#ff8fab] text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed">
              {processing ? "Procesando..." : "Confirmar Pago"}
            </button>
          </div>
        )}
        ...
        {paymentStep === "card" && (
          <div className="max-w-md mx-auto bg-white rounded-3xl p-8 shadow-lg border border-gray-100">
            <div className="text-center mb-6">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#2563eb] to-[#3b82f6] flex items-center justify-center mx-auto mb-4 shadow-lg">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <button type="button" onClick={() => setShowCardCvv(!showCardCvv)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400">
            ...
            <div className="bg-[#f0f4ff] rounded-2xl p-4 mb-6 flex justify-between items-center">
              <span className="text-sm text-gray-600">Total a pagar</span>
              <span className="font-bold text-xl text-[#ff6b9d]">S/.{total.toFixed(2)}</span>
            </div>

            <button disabled={processing} onClick={handleCardPay} className="w-full py-3 bg-gradient-to-r from-[#2563eb] to-[#3b82f6] text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all disabled:opacity-50">
        ...
        {paymentStep === "cash" && (
          <div className="max-w-md mx-auto bg-white rounded-3xl p-8 shadow-lg border border-gray-100 text-center">
            <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-[#22c55e] to-[#16a34a] flex items-center justify-center mx-auto mb-4 shadow-lg">
            ...
            {deliveryMethod === "pickup" ? (
              <>
                <p className="text-gray-500 text-sm mb-6">
                  ...
                  Podrás realizar el pago al momento de recogerlo en tienda.
                </p>
              </>
            ) : (
              <>
                <p className="text-gray-500 text-sm mb-6">
                  ...
                  Ten el monto exacto preparado para agilizar la entrega.
                </p>
              </>
            )}
            ...
            <div className="bg-[#f0fdf4] rounded-2xl p-4 mb-6 space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-[#2d2d2d]">{deliveryMethod === "pickup" ? "Recojo en tienda" : "Delivery"}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Total a pagar</span>
                <span className="font-bold text-lg text-[#ff6b9d]">S/.{total.toFixed(2)}</span>
              </div>
            </div>

            <button disabled={processing} onClick={() => handlePlaceOrder()} className="w-full py-3 bg-gradient-to-r from-[#ff6b9d] to-[#ff8fab] text-white font-semibold rounded-xl shadow-md hover:shadow-lg transition-all disabled:opacity-50">
